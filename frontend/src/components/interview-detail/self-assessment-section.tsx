...
export const SelfAssessmentSection = ({
    ...
    const [overallFeeling, setOverallFeeling] = useState<OverallFeeling | undefined>(
        interview.overall_feeling as OverallFeeling | undefined
    );
    const [wentWell, setWentWell] = useState(interview.went_well || '');
    const [couldImprove, setCouldImprove] = useState(interview.could_improve || '');
    ...
    // Auto-save on text field changes (30s debounce)
    useEffect(() => {
    ...
    // Immediate save for select/button changes
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-2">
                ...
                <Select
                    ...
                    onValueChange={(value) => handleOverallFeelingChange(value as OverallFeeling)}
                >
...
// Helper to generate summary for collapsed state
export const getSelfAssessmentSummary = (interview: Interview): string | null => {
    if (!interview.overall_feeling && !interview.confidence_level) {
        return null;
    }

    const parts: string[] = [];

    if (interview.overall_feeling) {
        const feeling = OVERALL_FEELINGS.find(f => f.value === interview.overall_feeling);
        if (feeling) {
            parts.push(feeling.label);
        }
    }

    if (interview.confidence_level) {
        parts.push(`${interview.confidence_level}/5`);
    }
